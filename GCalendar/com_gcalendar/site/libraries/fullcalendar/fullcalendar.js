...
function BasicView(element, calendar, viewName) {
	...
	function buildSkeleton(maxRowCnt, showNumbers) {
		// allon http://code.google.com/p/joomla-gcalendar/source/diff?spec=svn1003&old=999&r=1003&format=side&path=%2Ftrunk%2FJoomla_1_5_x%2Fcom_gcalendar%2Fsite%2Flibraries%2Ffullcalendar%2Ffullcalendar.js
		var s;
		var headerClass = tm + "-widget-header";
		var contentClass = tm + "-widget-content";
		var i, j, d = cloneDate(t.visStart);
		var table;
		...
		s =
			...
			"<tr>";
		if (wkn) {
			s += "<th width='25px' "+headerClass+"'>&nbsp;</th>";
		}
		for (i=0; i<colCnt; i++) {
			s +=
	...
	function updateCells(firstTime) {
		...
		bodyCells.each(function(i, _cell) {
			...
			if (wkn && date.getDay() == 1)
				cell.closest('tr').find('th').text(date.getWeek());
		});
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		s =
			...
			"<thead>" +
			"<tr>" +
			"<th class='fc-agenda-axis " + headerClass + "'>" + (opt('weekNumbers') ? formatDate(t.visStart, 'W') : '&nbsp;') + "</th>";
		for (i=0; i<colCnt; i++) {
			s +=
	...
	function updateCells() {
		...
		if (opt('weekNumbers')) {
			$(axisFirstCells[0]).text( formatDate(t.visStart, 'W') );
		}
